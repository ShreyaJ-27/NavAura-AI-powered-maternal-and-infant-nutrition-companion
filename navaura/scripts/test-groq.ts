import { callGroqVision } from '../lib/groq';
import fs from 'fs';
import path from 'path';

async function testGroq() {
  console.log('Testing Groq Vision API directly...');
  console.log('Model:', process.env.GROQ_VISION_MODEL || 'qwen/qwen3.6-27b');
  console.log('API Key configured:', Boolean(process.env.GROQ_API_KEY));

  // Create a minimal 1x1 valid JPEG file in memory
  // Minimal valid JPEG binary buffer
  const jpegBytes = Buffer.from([
    
  ]);

  const file = new File([jpegBytes], 'meal.jpg', { type: 'image/jpeg' });

  try {
    const result = await callGroqVision(file);
    console.log('✅ Groq Vision Call Succeeded!');
    console.log('Result:', JSON.stringify(result, null, 2));
  } catch (err: any) {
    console.error('Groq test error:', err.message);
  }
}

testGroq();
